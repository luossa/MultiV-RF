...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        model = self.model_dict[self.args.model].Fusion_model(self.args).float()
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            test_mae, test_mse = self.test()
            print("Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} Test mae: {4:.7f} Test mse: {5:.7f}".format(
                epoch + 1, train_steps, train_loss, vali_loss, test_mae, test_mse))
            folder_path = self.args.save_dir
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            f = open(folder_path + "result_long_term_forecast.txt", 'a')
            f.write('Epoch:{}, Train Loss:{}, Vali Loss:{}, mae:{}, mse:{}'.format(epoch + 1, train_loss, vali_loss, test_mae, test_mse))
            f.write('\n')
