...
def prompt_build(x, description, factor, seq_len, name):
    B, T, N = x.shape
    prompt = []
    # 构建提示词
    for b in range(B):  # 遍历每个样本
        prompt_ = (
            f"<|start_prompt|>Dataset description: Stock: {name}, {description} "
            f"<|start_prompt|>External factors: {factor} "
            f"Task description: forecast the next 30 day risk given the previous {str(seq_len)} steps information. "
        )
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        self.gpt2 = AccustumGPT2Model.from_pretrained('/gemini/pretrain3/gpt2', output_attentions=True,
                                                      output_hidden_states=True)  # loads a pretrained GPT-2 base model
        self.gpt2_fussion = AccustumGPT2Model.from_pretrained('/gemini/pretrain3/gpt2', output_attentions=True,
                                                              output_hidden_states=True)  # loads a pretrained GPT-2 base model
        self.gpt2_prompt = AccustumGPT2Model.from_pretrained("/gemini/pretrain3/gpt2", output_attentions=True,
                                                             output_hidden_states=True)
        self.tokenizer = GPT2Tokenizer.from_pretrained("/gemini/pretrain3/gpt2")
    ...
    def forecast(self, x, name):
        ...
        # create prompt
        prompt = prompt_build(x, self.content, self.factor, self.seq_len, name)
    ...
    def forward(self, x, name):
        if self.task_name == 'long_term_forecast' or self.task_name == 'classification':
            output = self.forecast(x, name)
        return output
