...
def vali_classify(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    all_preds = []
    all_trues = []
    total_loss = []
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, name) in enumerate(vali_loader):
            ...
            # 分类任务：直接调用模型，不需要 decoder 输入
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    outputs = model(batch_x, name)[0]  # 输出 (batch_size, n_classes)
            else:
            ...
            pred = outputs.detach()
            true = batch_y.detach()
            loss = criterion(pred, true)  # CrossEntropyLoss 输入：(batch_size, n_classes), (batch_size,)
    ...
    final_accuracy = accuracy_score(all_trues, all_preds)
    final_f1 = f1_score(all_trues, all_preds, average='weighted')
    ...
    model.train()
    return final_loss, final_accuracy, final_f1, df

def vali_regression(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    total_loss = []
    total_mae_loss = []
    all_preds = []  # 存储所有预测值
    all_trues = []
    ...
    df = pd.DataFrame({
            'Prediction': all_preds,
            'GroundTruth': all_trues
        })
    
    model.train()
    return total_loss, total_mae_loss, df
...
def test(args, accelerator, model, test_loader, criterion):
    total_loss = []
    total_accuracy = []
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
            batch_x = batch_x.float().to(accelerator.device)
            batch_y = batch_y.long().to(accelerator.device)  # 分类任务：batch_y 是类别索引
            batch_x_mark = batch_x_mark.float().to(accelerator.device)
            batch_y_mark = batch_y_mark.float().to(accelerator.device)
            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    if args.output_attention:
                        outputs = model(batch_x, batch_x_mark, None, None)[0]
                    else:
            ...
            accuracy = (pred_classes == true).float().mean()
...
def load_content(args):
    if 'ETT' in args.data:
        ...
        file = args.data
    with open('/gemini/code/SP500/Time-LLM/context/{0}.txt'.format(file), 'r') as f:
        content = f.read()
    return content